...
## SOCKS5 setup on your VPS (Ubuntu 22.04/24.04) — recommended

Below is a practical setup using **Dante (danted)**.
...
### 3) Create SOCKS5 user for authentication

```bash
sudo useradd -r -s /usr/sbin/nologin socksuser
...
Use this example (replace network interface if needed):
...
> Find your interface name with `ip a` (often `eth0`, `ens3`, or `enp1s0`).
...
If you use cloud firewall (Hetzner/DO/AWS/etc.), also allow inbound `1080` there.

### 7) Test SOCKS5 from your PC

From Windows PowerShell (or any machine with curl):

```powershell
curl --proxy socks5h://socksuser:YOUR_PASSWORD@YOUR_VPS_IP:1080 https://ifconfig.me
```

If it returns your VPS IP, SOCKS5 works.

### 8) Put values into extension
...
Then click **Save settings** → **Enable routing**.

---

## Optional hardening for SOCKS5 server

- Allow only your home/office IP in firewall (instead of `0.0.0.0/0`).
- Change default port from `1080` to custom high port.
- Use fail2ban and monitor logs:
...
### danted does not start
- Check interface name in `/etc/danted.conf` (`external: eth0` may be wrong).
- Check logs:
  ```bash
  sudo journalctl -u danted -n 200 --no-pager
  ```
